...
class SuperusSupabaseUser extends BaseAuthUser {
  SuperusSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> superusSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = SuperusSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
